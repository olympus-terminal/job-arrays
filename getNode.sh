...
job_info=$(sacct -j $job_id -o JobID,JobName,User,Partition,State,ExitCode,WorkDir,Nodelist --parsable2)
...
node_list=$(echo "$job_info" | awk -F'|' '{print $8}') # Get the node list
